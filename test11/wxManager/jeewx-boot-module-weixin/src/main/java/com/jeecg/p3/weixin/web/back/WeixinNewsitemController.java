...
//            if(!StringUtils.isEmpty(weixinNewsitem.getContent())){
//                String[] urls = VideoUtil.getUrls(weixinNewsitem.getContent());
//                //将视频下载到本地
//                if(null != urls && urls.length > 0 ){
//                    for(String url:urls){
//                        String s = VideoUtil.downloadVideo(url);
//                        String content = weixinNewsitem.getContent();
//                        weixinNewsitem.setContent(content.replace(url,s));
//                    }
//                }
//            }
    ...
    @RequestMapping(params = "imgUploadContent", method = RequestMethod.POST)
    @ResponseBody
    public  Map imgUploadContent(MultipartFile upfile) throws IOException {
        ...
        String uploadDir = CommonWeixinProperties.imgSave +CommonWeixinProperties.imgWx;
        ...
        attributes.put("url", CommonWeixinProperties.imgDomain+CommonWeixinProperties.imgMap+CommonWeixinProperties.imgWx+filename);
        String format = filename.substring(filename.lastIndexOf("/") + 1);
        attributes.put("name",filename);
        attributes.put("state", "SUCCESS");
        return attributes;
    ...
    public AjaxJson doUpload(MultipartHttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            while (it.hasNext()) {
                //根据文件名称取文件
                MultipartFile uploadify = request.getFile(it.next());
