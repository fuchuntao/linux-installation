...
import com.jeecg.p3.weixin.util.VideoUtil;
...
public class WeixinNewstemplateServiceImpl implements WeixinNewstemplateService {
  	private Logger log = LoggerFactory.getLogger(WeixinNewstemplateServiceImpl.class);
	...
	public final static String upload_group_news_url2 = "https://api.weixin.qq.com/cgi-bin/material/add_news?access_token=ACCESS_TOKEN";
	@Override
...
//					JSONObject object = VideoUtil.uploadVideo(url, newsItem.getImagePath(), "这是标题", "这是描述");
//					log.info("ogject:{}",object);
					String sss = this.uploadPhoto(newsItem.getImagePath(),jwid);
					log.info("媒体id：{}",sss);
					baseGraphic.setThumb_media_id(sss);
					//baseGraphic.setMedia_id(sss);
	...
	private String updateContent(String content, String jwid) {
		if (content != null) {
			...
			String[] videoUrls = VideoUtil.getUrls(content);
			if(videoUrls != null && videoUrls.length > 0){
				for(String url:videoUrls){
//					if(url.indexOf(CommonWeixinProperties.imgSave) == -1){
//						continue;
//					}
					int i = url.lastIndexOf("/");
					try {
						WxMediaResponse video = VideoUtil.uploadMediaFile(accessToken, url.substring(0, i), url.substring((i + 1)), "video");
						if(null != video){
							System.out.println("返回media_id结果："+video);
							String newUrl = VideoUtil.getVideoUrl(accessToken,video.getMedia_id());
							System.out.println("视频新url："+newUrl);
							content = content.replace(url,newUrl);
						}
						//content.replace(url,video.)
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
