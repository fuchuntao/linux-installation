...
import com.jeecg.p3.weixin.vo.WxUserInfoDTO;
			...
			//【微信触发类型】事件推送
				...
				if (eventType.equals(MessageUtil.EVENT_TYPE_SUBSCRIBE)) {
					try {
						...
						wxUserInfoDTO.setOpenId(weixinMessageDTO.getFromUserName());
						//weixinMessageDTO.get
						if(weixinMessageDTO.getKey().indexOf("qrscene_")!=-1){
							...
							wxUserInfoDTO.setUserId(Long.valueOf(sceneId));
						}
						String toJson = new Gson().toJson(wxUserInfoDTO);
						rabbitTemplate.convertAndSend("save_user_openid",toJson);
				...
				// 取消订阅
					...
					try {
                        ...
                        wxUserInfoDTO.setOpenId(weixinMessageDTO.getFromUserName());
                        String toJson = new Gson().toJson(wxUserInfoDTO);
                        rabbitTemplate.convertAndSend("delete_user_openid",toJson);
						System.out.println("mq发送成功！");
					} catch (Exception e) {
						e.printStackTrace();
