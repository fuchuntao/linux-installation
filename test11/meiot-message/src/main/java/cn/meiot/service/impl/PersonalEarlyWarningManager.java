...
@Slf4j
public class PersonalEarlyWarningManager implements AlarmVoManager {
    @Autowired
    ...
    @Value("${wx.early_warning.template_id}")
...
//            appUserFaultMsgAlarm.setEquipmentAlias(deviceAlias);
            appUserFaultMsgAlarm.setEvent(EarlyWarning.get(e.getEvent()));
...
//            appUserFaultMsgAlarm.setUserId(userId);
            ...
            appUserFaultMsgAlarm.setType(2);
...
//            appUserFaultMsgAlarm.setMsgContent(deviceAlias + "   " + switchAlias + FaultMsgContentEnum.getContent(EarlyWarning.get(e.getEvent())));
            ...
            log.info("预警内容:{}", appUserFaultMsgAlarm);
            for (int i = 0; i < userIds.size(); i++) {
                ...
                try {
                    ...
                    WXMessageVo.Entry first = wxMessageVo.new Entry("您的设备发生" + FaultTitleEnum.getTitle(EarlyWarning.get(e.getEvent())) + ",请尽快处理。检修设备时，请注意安全，谨防触电。", "#363636");
...
//                    if (null != authUserBo) {
                        ...
                        jPushClientExample.sendMsg(Arrays.asList(String.valueOf(userId)), deviceAlias + "发生" + FaultTitleEnum.getTitle(EarlyWarning.get(e.getEvent())) + ",请尽快处理", FaultTitleEnum.getTitle(EarlyWarning.get(e.getEvent())),
                                userAlarm.getMsgContent(), JpushTypeEnum.NOTIFICATION.value(), pushMap, 5);
//                    }
                } catch (Exception jke) {
                    log.error("极光推送失败:{}", e);
                    log.info("极光推送失败:{}", e);
